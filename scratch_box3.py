...
if len(tk1) > 0:
    cos_score1,_ = dc.compare_spike_trains(tk1,tk_true,sc,1/T,tau)
    print(f'No box: {cos_score1}')

if len(tk2) > 0:
    cos_score2,_ = dc.compare_spike_trains(tk2,tk_true,sc,1/T,tau)
...
for idx1,n in enumerate(noises):
    for idx2, s in enumerate(shutters):
        #scores,scores_box,snr = get_mean_score(s,n,repeats = 50)
