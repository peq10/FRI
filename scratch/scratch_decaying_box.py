...
#apply window
window = scipy.signal.windows.hann(len(n_vec))
window2 = scipy.signal.windows.tukey(len(n_vec),alpha = 0.5,sym = True)

#plt.figure()
plt.plot(t_n,z_n/z_n.max(), label = '$z_n$')
plt.stem(t_k,a_k,use_line_collection = 'True',label = 'True diracs')

z_n *= window

plt.plot(t_n,z_n/z_n.max(), label = '$z_n$ windowed')
...
tk += (n_vec[-1] - 1/2)*T #-1*shutter_length

plt.stem(tk,ak,'r',use_line_collection = 'True',label = 'Recovered diracs')
