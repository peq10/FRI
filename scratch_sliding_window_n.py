...
x += np.random.normal(scale = noise_level,size = len(x))
...
all_tk1,all_ak1 = ee.window_extract(z_n,t_n,c_m_n,n_vec,alpha_vec,fixed_K=None)

win_len2 =16
phi,t_phi,c_m_n,n_vec,alpha_vec = ges.decaying_exp_filters(win_len2, T, tau)
z_n,t_n = ee.convert_exponential_to_dirac(t,x,phi,t_phi,tau)
all_tk2,all_ak2 = ee.window_extract(z_n,t_n,c_m_n,n_vec,alpha_vec,fixed_K=1)
